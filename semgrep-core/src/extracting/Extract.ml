...
let extract_collect_and_comb erule_table xtarget rule_ids matches =
  ...
  |> List.map (fun matches -> nonempty_to_list matches)
  |> List.map
  ...
  |> List.filter_map (function
       | [] -> None
       | (r, _) :: _ as xs -> Some (r, List.map snd xs))
  |> List.map (fun (r, mvals) ->
         ( r,
           List.filter_map
             (fun mval ->
  ...
  |> List.map (fun (r, offsets) ->
         ...
         |> List.fold_left
              ...
              []
         |> List.rev
         |> List.map (fun { start_pos; start_line; start_col; end_pos } ->
